...
void GestionVague()
{
    ...
    TempoRegen=105-(Difficulte<<1)-(NumeroZone<<2);
	...
	if (spr2->Phase)
	{
	    if (!Difficulte) NombreIASceneMax=1;
	    else NombreIASceneMax=2;
	}
}
    ...
    if (!spr->SpriteDYN)
    {
        ...
        if (spr->AirUnit)
        {
            ...
            if (!spr->Phase)
            {
                ...
                {
                    spr->Visible=1;
                    if (!VisCamX) spr->CoordX=SpriteREF->CoordX-FIX32(200);
                    if (VisCamX==2) spr->CoordX=FIX32(1600);
